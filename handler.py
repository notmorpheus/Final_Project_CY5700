...
def check_files_opened(process_id):
    ...
    now = 0
    bpf_string= f"tracepoint:syscalls:sys_enter_openat /pid == {process_id}"
...
if __name__=='__main__':
    ...
    for line in run_program(command):
        ...
        if(profiled_data_list is not None):
            ...
            files_opened = check_files_opened(pid)
            tcp_connections_accpeted = check_tcp_accept(pid)
            tcp_connections_made = check_tcp_connect(pid)
            system_calls_made = check_system_calls(pid)
